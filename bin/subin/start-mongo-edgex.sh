...
cp ../consulconf/mongo.json /nfs/data-go/consulconfig/
cp ../consulconf/rule-template.drl /nfs/data-go/rulestemplates/
echo "清除复位consul与db文件结束"

source ./create-service.sh consul ;
sleep 5;
source ./create-service.sh configmongo ;
sleep 12;
#删除configseed，防止k8s不断重启它
source ./delete-service.sh configmongo ;
sleep 1;

source ./create-service.sh mongodb ;
sleep 8;
...
echo ""
echo "Edgex Foundry （mongodb版） 启动完毕！"
...
echo "masterIP edgex-mongo"
echo "================================"
echo ""
echo ""
echo "如果装了dashboard，访问地址：https://masterIP:30009"
echo ""
echo "停止edgex服务使用： ./stop-edgex.sh mongo"
echo ""